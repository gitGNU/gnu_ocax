...
(function ($) {
	$.fn.ocaxjqplot = function( options ) {
		var options = $.extend({
			source: '',
			budget: 0,
		}, options);
		...
		$.fn.ocaxjqplot.preLoadInitialBudget = function(graphData) { createGraph(this, options.budget, graphData) };
		
		getPie(options.budget, this);	
	}

}( jQuery ));

function getSource(el){
	container = $(el).closest('.ocaxjqplot');
	return $(container).ocaxjqplot.getSource();
}
...
function getPie(budget_id, clicked_el){
	...
	$.ajax({
		url: source+'/budget/getPieData/'+budget_id,
		type: 'GET',
		dataType: 'json',
		beforeSend: function(){ /*$('.loader_gif').hide(); $(loading_gif).show();*/ },
		complete: function() { /*$('.loader_gif').hide(); */},
		success: function(data){
			createGraph(getGraphGroup($(clicked_el)), budget_id, data);
		},
		error: function() {
...
function createGraph(group, budget_id, data){
